...
@section('content')
    <h1 class="text-4xl font-bold">Create New Product</h1>
    @if (session('message'))
        {{ session('message') }}
    @endif
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-5">
            ...
            <div class="flex">
                ...
                <input type="text" id="website-admin" class="rounded-none rounded-e-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5 " placeholder="150000" name="price" value={{ old('price') }}>
              </div>
              @error('price')
                <p class="text-red-500">{{ $message }}</p>
              @enderror
        ...
        <div class="mb-5">
            ...
            <input type="text" id="base-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" name="stock" value={{ old('stock') }}>
            @error('stock')
              <p class="text-red-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-5">
            ...
            <textarea id="message" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Leave a comment..." name="description">{{ old('description') }}</textarea>
            @error('description')
              <p class="text-red-500">{{ $message }}</p>
            @enderror
        </div>
        <button type="submit" class="btn bg-gray-700 text-white px-3 py-2 rounded">Create New Product</button>
    </form>
@endsection